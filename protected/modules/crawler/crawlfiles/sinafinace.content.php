...
class sinafinaceContent extends contentCrawlerAbstract 
{
    ...
    {
        $dom = $this->htmldom->find('div[id="J_Article_Wrap"]',0);
        if (empty($dom)){
            return false;
        }
        $this->contentdom = $dom;
    ...
    protected function getCategory() {
        $dom = $this->htmldom->find('.blkBreadcrumbLink a',1);
        if (empty($dom)){
            return false;
        }
    ...
    protected function getMedia() {
        ...
        if (empty($dom))
            $dom = $this->contentdom->find('span[id="media_name"]',0);
        if (empty($dom))
            return false;
        $media = $dom->innertext;
    ...
    protected function getTime() {
        $dom = $this->contentdom->find('span[id="pub_date"]',0);
        if (empty($dom)){
            return false;
        }
        $time = $dom->innertext;
