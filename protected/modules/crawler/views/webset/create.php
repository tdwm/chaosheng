        ...
        'enableAjaxValidation'=>false,
                ...
                <div class="row-fluid ">
                    <?php
                        if($this->isManager()) {
                            ...
                            echo $form->textFieldRow($model, 'uid', array('placeholder' => '用户id','hint'=>'', 'style' => 'width: 98%')); 
                            echo '</div>';
                        }
                    ?>
                    <div class="span2" >
            ...
            <div class="row-fluid ">
                <div class="span2" >
                    <?php
                        if($model->secretkey) {
                            //echo CHtml::textField('secretkey', $model->secretkey,array('readonly'=>true));
                           echo  '<label for="CollectsWeb_secretkey">密钥</label> <span>'.$model->secretkey.'</span>';
                            //echo $form->textFieldRow($model,'secretkey',array('id'=>'secretkey','name'=>'secretkey','readonly'=>true));
                        }
                    ?>
                </div>
                <div class="span2 row" >
                    <?php
                        if($this->isManager()){
                            echo $form->textFieldRow($model,'secretkey',array('id'=>'secretkey','class'=>'inline'));
                            $this->widget('bootstrap.widgets.TbButton',array(
                                'type'=>'primary',
                                'label' => $model->secretkey?'修改密钥':'生成密钥',
                                'htmlOptions'=>array(
                                    'class'=>'inline',
                                    'onClick'=>'
                                        if(confirm("确定操作么？")){
                                            $.post("'.$this->createUrl('secretkey',array('id'=>$model->id)).'",function(r){
                                                $("#secretkey").val(r);
                                            }) ;
                                        }
                                    ',
