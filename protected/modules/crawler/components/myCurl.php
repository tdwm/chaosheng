...
class myCurl
{
     ...
     * @return string, false on failure
     */
    public function get($url, $timeout = 50)
    {
        if ( is_array($url) )
        {
            $getone = false;
    ...
    protected function _create($url,$timeout)
    {
        ...
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT_MS, myCurl::$connecttimeout_ms);
     ...
     * @return Array
     */
    protected function request_urls($urls, $timeout = 20)
    {
        # 去重
            ...
            while ( true==($done = curl_multi_info_read($mh)) )
            {
                foreach ( $listener_list as $done_url=>$listener )
                {
                    if ( $listener === $done['handle'] )
                    {
                        ...
                        {
                            throw new CHttpException(400,'URL:'.$done_url.' ERROR,TIME:' . $this->http_data[$done_url]['time'] . ',CODE:' . $this->http_data[$done_url]['code'] );
                        ...
                        else
                        {
                            ...
                            //throw new CHttpException(400,'URL:'.$done_url.' OK.TIME:' . $this->http_data[$done_url]['time'] );
                        }
