...
class attachment {
	...
	function download($field, $value,$watermark = '0',$ext = 'gif|jpg|jpeg|bmp|png', $absurl = '', $basehref = '')
	{
        ...
        $upload_url = $domain.'/'.$this->upload_dir.'/';
		$this->field = $field;
		$dir = date('Y/md/');
		$uploadpath = $upload_url.$dir;
		$uploaddir = $this->upload_root.$dir;
		$string = myFunc::new_stripslashes($value);
        ...
        //var_dump($remotefileurls);exit;
		$oldpath = $newpath = array();
        //$mycurl = new myCurl();
       // $images = $mycurl->get($remotefileurls);
        foreach($remotefileurls as $url){
            $image = HttpClient::quickGet($url);
            if (!$image) continue;
            $oldpath[] = $url;
            $fileext = myFunc::fileext($url);
            ...
            //$downloadedfile = array('filename'=>$filename, 'filepath'=>$filepath, 'fileext'=>$fileext);
            $aid = $this->add($downloadedfile);
            $this->downloadedfiles[$aid] = $filepath;
        }
        /*
        foreach($images as $url => $ascData){
            $oldpath[] = $url;
            if ($ascData == false) continue;
            $fileext = myFunc::fileext($url);
            $file_name = basename($url);
            $filename = $this->getname($fileext);
            $newfile = $uploaddir.$filename;
	...
	function add($uploadedfile) {
        $att = new Attachments();
		$att->nodeid = $this->nodeid;
		$att->status =  1;
		$att->filepath =  $uploadedfile['filepath'];
		$att->fileext =  $uploadedfile['fileext'];
		$att->authcode = md5($uploadedfile['filepath']);
		$att->filename = strlen($uploadedfile['filename'])>49 ? $this->getname($uploadedfile['fileext']) : $uploadedfile['filename'];
		$att->isimage = in_array($uploadedfile['fileext'], $this->imageexts) ? 1 : 0;
        $att->save();
