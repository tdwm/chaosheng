...
class CategoriesController extends ACiiController
{
	...
	public function actionSave($id=NULL)
	{
		...
		if(isset($_POST['Categories']))
		{
            ...
            $model->ifcreateTable = Cii::get($_POST['Categories'], 'ifcreateTable', 0);
			if($model->save())
			{
    ...
    public function checkDelete($id)
    {
        ...
        $model=$this->loadModel($id);
        if($model->ifcreateTable == 1) {
            $content_table = 'content_'.$model->slug;
            $command = Yii::app()->db->createCommand("select count(id) as count_num from `$content_table` ");  
            $rows = $command->queryRow(); //结果数组，多行的内容  
            if($rows['count_num'] > 0) {
                Yii::app()->user->setFlash('error', '此分类下有文章不能够删除.');
                return false;
            }  
        } 
        return true;
